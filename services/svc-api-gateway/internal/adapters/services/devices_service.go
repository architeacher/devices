...
type (
	// DevicesService implements the DevicesService and HealthChecker interfaces using gRPC.
	...
	ClientOption func(*DevicesService) error
)
...
// NewDevicesService creates a new gRPC client for the devices service.
func NewDevicesService(cfg *config.ServiceConfig, opts ...ClientOption) (*DevicesService, error) {
	...
	if grpcClientConfig.CircuitBreaker.Enabled {
		client.cb = gobreaker.NewCircuitBreaker[any](gobreaker.Settings{
			Name:        "devices-service",
			MaxRequests: uint32(grpcClientConfig.CircuitBreaker.MaxRequests),
			Interval:    grpcClientConfig.CircuitBreaker.Interval,
			Timeout:     grpcClientConfig.CircuitBreaker.Timeout,
			ReadyToTrip: func(counts gobreaker.Counts) bool {
				return counts.ConsecutiveFailures >= uint32(grpcClientConfig.CircuitBreaker.FailureThreshold)
			},
		})
	}
...
func (c *DevicesService) Close() error {
	if c.conn == nil {
		return nil
	}
...
func (c *DevicesService) CreateDevice(ctx context.Context, name, brand string, state model.State) (*model.Device, error) {
...
func (c *DevicesService) GetDevice(ctx context.Context, id model.DeviceID) (*model.Device, error) {
...
func (c *DevicesService) ListDevices(ctx context.Context, filter model.DeviceFilter) (*model.DeviceList, error) {
...
func (c *DevicesService) UpdateDevice(ctx context.Context, id model.DeviceID, name, brand string, state model.State) (*model.Device, error) {
...
func (c *DevicesService) PatchDevice(ctx context.Context, id model.DeviceID, updates map[string]any) (*model.Device, error) {
...
func (c *DevicesService) DeleteDevice(ctx context.Context, id model.DeviceID) error {
...
func (c *DevicesService) Liveness(ctx context.Context) (*model.LivenessReport, error) {
...
func (c *DevicesService) Readiness(ctx context.Context) (*model.ReadinessReport, error) {
...
func (c *DevicesService) Health(ctx context.Context) (*model.HealthReport, error) {
	checks := make(map[string]model.DependencyCheck)
	now := time.Now().UTC()
...
func (c *DevicesService) executeWithCircuitBreaker(_ context.Context, fn func() (any, error)) (any, error) {
	if c.cb == nil {
		return fn()
	}
...
func WithConnection(conn *grpc.ClientConn) ClientOption {
	return func(c *DevicesService) error {
		if c.conn != nil {
			c.conn.Close()
		}
